...
选择`循环节点`，打开`属性编辑工具栏`，看到循环节点上可以设置[循环变量](../variable/loop-type.md)，包含`自身循环变量`和`外部循环变量`两种：
...
+ 循环圈节点
  ...
  参与循环的所有题目节点。`循环圈节点`中的所有节点，都能使用[循环变量](../variable/loop-type.md)，代表每次重复针对`循环圈节点`提问时使用的内容，这样就能达到通过变量展示不同内容的效果。
