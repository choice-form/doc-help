...
> 关于模块的保存，请参照[节点组与模块](../groups/concept.md)
...
配合`Ctrl`、`Shift`（Mac系统中为`⌘`、`⇧`）键点击列表可以选择多个媒体资源。
...
点击详情栏中的瞄准图标，可以定位当前媒体文件在列表中的位置，这个操作在媒体文件较多，而且当前媒体文件不在滚动条可视区域内时是非常有用的，点击旁边的删除图标，则会删除该媒体资源，如果该媒体资源被其他对象引用了，则删除之前会弹出确认框。
